...
description: >
Define success, verify, output format, AND explicit failure conditions in a single structured contract. The agent treats it as a spec, not a suggestion. Triggers on "prompt contract", "write a contract", "define success criteria", "failure conditions", "spec this out", or /prompt-contracts. Also triggers on phrases like "what does done look like", "define the contract", "structured spec", or "goal constraints format failure".
